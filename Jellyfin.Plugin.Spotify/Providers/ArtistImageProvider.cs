...
/// <summary>
/// Supplies artist artwork from Spotify.
/// </summary>
public class ArtistImageProvider : IRemoteImageProvider
{
    ...
    public async Task<IEnumerable<RemoteImageInfo>> GetImages(BaseItem item, CancellationToken cancellationToken)
    {
        ...
        {
            var found = await _catalog.GetArtistAsync(id, cancellationToken).ConfigureAwait(false);
            return ToImageInfos(found is null ? [] : found.Images);
        }

        _logger.LogDebug("Searching Spotify artist artwork for {Term}", artist.Name);
        var artists = await _catalog.SearchArtistsAsync(artist.Name, cancellationToken).ConfigureAwait(false);
        return artists.SelectMany(candidate => ToImageInfos(candidate.Images)).ToList();
    }
