...
#   dist/spotify_<version>_<abi-name>.zip        archive
#   dist/spotify_<version>_<abi-name>.zip.md5    checksum (Jellyfin manifests use MD5)
set -euo pipefail
...
for target in "${TARGETS[@]}"; do
  ...
  find "$outdir" -type f \
    ! -name 'Jellyfin.Plugin.Spotify.dll' \
    ! -name 'Jellyfin.Plugin.Spotify.pdb' \
    -delete
  ...
  archive="$DIST/spotify_${VERSION}_${name}.zip"
